---
title: "What's New"
slug: "whats-new"
description: "Recent additions to Beijerterm - new glossaries, terms, and content"
---

<!--
TEMPLATE FOR ADDING NEW ENTRIES:

## [Month] [Year]

### New Terms Added
- **[term-slug](/terms/term-slug)** (NL→EN) - Brief description

### New Glossaries Added
- **[Glossary Name](/glossaries/glossary-slug)** - Description (X entries)

### Updated Glossaries
- **Glossary Name** - What was added/changed

TIP: Run `python scripts/add_whats_new_entry.py` for interactive entry creation!
-->

# 📰 What's New in Beijerterm

This page tracks **new glossaries, terms, and content** added to Beijerterm. For website code changes, see the [GitHub changelog](https://github.com/michaelbeijer/beijerterm/blob/main/CHANGELOG.md).

---

## January 2026

### New Terms Added

- **[splitpen](/terms/splitpen)** (NL→EN) - Splitting pen (patent drafting tool)
- **[vergisting](/terms/vergisting)** (NL→EN) - Fermentation, digestion (biogas/organic waste processing)

### Updated Glossaries

- **Patent Mechanics** - Added 15+ new mechanical engineering terms
- **Biogas Terminology** - Expanded fermentation and waste treatment terms

---

## December 2025

### Website Launch 🎉

- **583,000+ term entries** across multiple Dutch-English glossaries
- **Full-text search** powered by Pagefind
- **6 color themes** for comfortable reading
- **Mobile-responsive** design for on-the-go translation work

### Initial Glossaries

- Patent terminology (mechanics, chemistry, biology)
- Technical terminology
- General translation resources

---

## How to Contribute

Found a term that should be added? Have corrections or suggestions?

1. **Open an issue** on [GitHub](https://github.com/michaelbeijer/beijerterm/issues)
2. **Submit a pull request** with your additions
3. **Email me** at info@michaelbeijer.co.uk

All contributions are released under CC0 (public domain) and credited to you (if desired).

---

<div class="callout callout-info">
💡 <strong>Tip:</strong> Want to stay updated? Watch the <a href="https://github.com/michaelbeijer/beijerterm">GitHub repository</a> for notifications of new terms and glossaries.
</div>
